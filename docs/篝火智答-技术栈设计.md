...
- **智能应急咨询（P0）**：基于 RAG 检索增强生成，为孤独症人士突发行为/情绪危机提供即时结构化应急方案
- **个性化档案库（P0）**：家属与老师共建行为-情绪档案，标签体系驱动检索过滤
...
```
家属输入行为描述 → Taro 小程序 → Nginx → FastAPI /api/v1/consult/stream
...
**接口设计**：
- `POST /api/v1/profiles` — 创建/更新个人档案（家属 + 老师共建）
