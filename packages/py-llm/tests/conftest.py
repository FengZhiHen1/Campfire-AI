"""py-llm 测试共享夹具、工厂函数和 mock 客户端。

所有 mock 客户端均实现 LLMClientContract 钩子，
供前置条件、重试、边界和集成测试套件共用。
"""
...
def make_rate_limit_error(status_code: int = 429) -> RateLimitError:
    """创建可通过 isinstance 检查的 RateLimitError。"""
...
# ============================================================================
# 测试 mock: NormalClient —— 用于前置条件/边界/正常路径测试
# ============================================================================
...
class NormalClient(LLMClientContract):
    ...
    def inject_stream_error(self, exc: Exception) -> None:
        """注入流式错误——下次 _do_create_stream 将抛出此异常。"""
        self._stream_error = exc

    def inject_completion_error(self, exc: Exception) -> None:
        """注入非流式错误——下次 _do_create_completion 将抛出此异常。"""
...
# ============================================================================
# 测试 mock: ErrorSequenceClient —— 用于重试循环行为测试
# ============================================================================
...
class ErrorSequenceClient(LLMClientContract):
    ...
    errors 字典: {attempt_index: Exception} —— 在对应序号的尝试（0 起始）上
    抛出指定的异常。未配置异常的 attempt 正常返回预设数据。
    """
...
# ============================================================================
# 测试 mock: SpyClient —— 用于验证后置条件/钩子调用
# ============================================================================
