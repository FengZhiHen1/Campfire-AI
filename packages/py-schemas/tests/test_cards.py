...
from py_schemas.cards import CardCreateRequest, CardExtractionResult, CardResponse, CardUpdate
...
class TestCardCreateRequest:
    ...
    def test_empty_title(self):
        with pytest.raises(ValidationError):
            CardCreateRequest(
                title="", scenario="x", behavior_type="自伤",
                age_range=[3, 12], severity="重度", scene="家庭",
                ebp_labels=["强化"], family_category="危机安全",
                immediate_action="a", comforting_phrase="b",
                observation_metrics="c", medical_criteria="d",
                evidence_level="e", contraindications="f",
            )

    def test_empty_immediate_action(self):
        with pytest.raises(ValidationError):
            CardCreateRequest(
                title="测试", scenario="x", behavior_type="自伤",
                age_range=[3, 12], severity="重度", scene="家庭",
                ebp_labels=["强化"], family_category="危机安全",
                immediate_action="", comforting_phrase="b",
                observation_metrics="c", medical_criteria="d",
                evidence_level="e", contraindications="f",
            )
...
class TestCardExtractionResult:
    def test_valid(self):
        card = CardCreateRequest(
            title="卡片", scenario="场景", behavior_type="自伤",
            age_range=[3, 12], severity="重度", scene="家庭",
            ebp_labels=["强化"], family_category="危机安全",
            immediate_action="a", comforting_phrase="b",
            observation_metrics="c", medical_criteria="d",
