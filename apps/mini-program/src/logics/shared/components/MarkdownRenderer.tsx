...
function InlineText({ token, blockType }: { token: InlineToken; blockType: string }) {
  if (token.type === 'bold') {
    return <Text className={`md-${blockType} md-bold`}>{token.content}</Text>;
  }
  if (token.type === 'italic') {
    return <Text className={`md-${blockType} md-italic`}>{token.content}</Text>;
  }
  if (token.type === 'code') {
    return <Text className={`md-${blockType} md-inline-code`}>{token.content}</Text>;
  }
  return <Text className={`md-${blockType}`}>{token.content}</Text>;
}
    ...
    <View className="md-root">
      {blocks.map((block, idx) => {
        switch (block.type) {
          ...
          case 'list-item':
            return (
              <View key={idx} className="md-list-item">
                <Text className="md-list-item__bullet">•</Text>
                <View className="md-list-item__body">
                  {block.tokens.map((t, ti) => (
          ...
          case 'code-block':
            return (
              <View key={idx} className="md-code-block">
                <Text className="md-code-block__text">{block.tokens[0]?.content ?? ''}</Text>
              </View>
            );
