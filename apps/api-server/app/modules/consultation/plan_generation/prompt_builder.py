...
_SYSTEM_PROMPT_TEMPLATE: str = (
    ...
    "**正确输出（首个字符必须是左花括号）**：\n"
    '{"即时安全干预动作":["措施1","措施2"],"情绪安抚话术":["话术1","话术2"],"后续观察指标":["指标1","指标2"],"就医判断标准":["标准1","标准2"]}\n\n'
    "**严禁输出（以下格式均为错误）**：\n"
    "- 以 ```json 或 ``` 开头或结尾\n"
    ...
    "## 引用规则\n"
    "- 引用参考案例时，在建议文本末尾使用 [N] 格式标注（如 [1]、[2]）\n"
    "- **禁止**使用未在参考案例中分配的序号\n"
    "- 若无参考案例，不得包含任何 [N] 格式标记\n\n"
    "## 专业术语解释规则\n"
    ...
    "- 无参考案例时，在第一条建议中明确告知：当前无匹配的干预案例参考，以下建议基于通用专业知识\n\n"
    "## 确定性与语气约束\n"
