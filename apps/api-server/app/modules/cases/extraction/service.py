...
| age_range | 始终用整数数组，如 [6, 12]；禁止加引号、单位或范围字符串 |
...
age_range 始终用整数数组，格式为 [min, max]，如 [6, 12]。caution_notes 若无可填空字符串。contraindications 若无可填 "无"。

## 输出格式示例（重点关注 age_range）
...
```
错误原因：`age_range` 是字符串 `"4-10"`，必须是整数数组 `[4, 10]`。
"""
...
class ExtractionService(ExtractionServiceContract):
    ...
    async def _do_extract(
        ...
        for i, raw in enumerate(cards_data):
            # 先对 LLM 常见格式错误做归一化，再进入严格校验
            raw["age_range"] = self._normalize_age_range(raw.get("age_range"))
    ...
    @staticmethod
    def _normalize_age_range(value: Any) -> list[int] | None:
        """把 LLM 常见的 age_range 乱格式归一化为 [min, max] 整数数组。
        ...
        # 范围字符串：6-12、6~12、6到12、6岁-12岁 等
        # 先去掉中文单位，再按分隔符拆分
        cleaned = re.sub(r"[岁岁个月年]|\s+", "", text)
        for sep in ("-", "~", "到", "—", "–"):
            if sep in cleaned:
